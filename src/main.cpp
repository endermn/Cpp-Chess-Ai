...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *win =
      SDL_CreateWindow("Chess", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                       SQUARE_SIZE * 10, SQUARE_SIZE * 8, SDL_WINDOW_RESIZABLE);
  SDL_ShowWindow(win);
  ...
  while (true) {
    if (time_white.count() <= 0 || time_black.count() <= 0) {
      SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "Game Ended",
                               "Game ended, time ran out", nullptr);
      exit(0);
    }
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_MOUSEBUTTONDOWN: {
        ...
        if (event.button.button != SDL_BUTTON_LEFT ||
            sync.position.turn == engine_color)
          break;
        ...
        if (possible_moves.get(pos)) {
          ...
          if (sync.position.board[pos.y][pos.x].has_value())
            SDL_QueueAudio(capture_sound.device, capture_sound.buffer,
                           capture_sound.size);
          else
            SDL_QueueAudio(move_sound.device, move_sound.buffer,
                           move_sound.size);
      ...
      case SDL_KEYDOWN:
        ...
        case SDLK_LEFT:
          if (!sync.is_thinking && old_positions.size() > 0) {
            ...
            rollback_position(old_positions, possible_moves.bits, src_pos, sync,
                              time_black, time_white);
          }
          break;
        case SDLK_RIGHT:
          if (event.key.keysym.sym == SDLK_RIGHT && !sync.is_thinking &&
              new_positions.size() > 0) {
            old_positions.push_back({sync.position, time_black, time_white});
            rollback_position(new_positions, possible_moves.bits, src_pos, sync,
                              time_black, time_white);
          }
          break;
        case SDLK_F5:
    ...
    canvas.draw(time_black, time_white, sync.position.board, possible_moves,
                sync.position.turn);
  }
